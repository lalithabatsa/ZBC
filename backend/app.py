...
def parse_excel(file_path):
    ...
    print(f"Parsing {filename}, formatted date: {filename_date}")
    
    xl = pd.ExcelFile(file_path)

    # Find 'ex-works' sheet case-insensitively
    ...
    # Heuristic: Find rows that look like table headers
    for i in range(len(df)):
        ...
        if "Sr.No." in row_values and "Pricing Zone" in row_values and "State" in row_values:
            ...
            grades = [h for h in row_values[state_idx+1:] if h and "Unnamed" not in h and h != "None"]
            
            # Now look UPWARDS for metadata (Date, Ref, Material/Location)
            ...
            for k in range(max(0, i-6), i):
                ...
                # Look for Location/Material keywords
                if "Domestic Prices for" in prev_row or "Prices for" in prev_row:
                    ...
                    if loc_match:
                        ...
                        # Use the first word as the location (e.g. "Hazira")
        ...
        # Check if this row is the table header (Sr.No., Pricing Zone, State)
        if "Sr.No." in row_values and "Pricing Zone" in row_values and "State" in row_values:
            
            # Explicitly ignore HDPE tables as requested
            if meta.get("table_material") == "HDPE":
                continue

            header = row_values
            # The grades are the columns after "State"
            state_idx = header.index("State")
            grades = [h for h in header[state_idx+1:] if h and h != "None" and "Unnamed" not in h]
            
            # Now parse rows below the header
            ...
            # Skip the rows we just parsed
            i = j
            
    return records

def load_all_data():
    # Use recursive glob to find files in subfolders
    files = glob.glob(os.path.join(PRICING_FOLDER, "**/*.xlsx"), recursive=True)
    ...
    for f in files:
        ...
        # Ensure only Reliance files are processed
        if "Reliance" not in os.path.basename(f):
            continue

        try:
            data = parse_excel(f)
            # Add folder info as category if possible
            folder_name = os.path.basename(os.path.dirname(f))
            for d in data:
                t_mat = d.get("table_material")
                if t_mat in ["LDPE", "LLDPE"]:
                    d["folder_key"] = t_mat
                else:
                    d["folder_key"] = folder_name
